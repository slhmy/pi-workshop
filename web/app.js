...
function createUiIcon(name, className = "") {
  ...
  use.setAttribute("href", `/icons.svg?v=2#${name}`);
  icon.append(use);
  return icon;
}
...
function createMessageElement(message, { streaming = false, persisted = false } = {}) {
  ...
  author.textContent = message.role === "user" ? "You / instruction" : "Pi / response";
  renderMessageBody(body, message.text);
...
function reconcileTransientMessages(messages) {
  const assistantTexts = new Set(messages.filter((message) => message.role === "assistant").map((message) => message.text));
  const remaining = [];
...
function toolActivityDetail(event) {
  ...
  return event.toolName === "bash" ? "Executing in workspace" : "Tool running";
}
